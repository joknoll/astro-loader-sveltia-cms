...
import { readCmsConfig, resolveCollection } from "../src/config.ts";
...
describe("sveltiaLoader — object form", () => {
  ...
  it("createSchema() returns a schema that validates data matching the fields", async () => {
    ...
    const s = schema as unknown as { safeParse: (v: unknown) => { success: boolean } };
  ...
  it('createSchema() excludes body for the auto-detect "frontmatter" format', async () => {
    const col: EntryCollection = {
      name: "posts",
      folder: "src/content/posts",
      format: "frontmatter",
